    ...
    class GoodsSearchModel extends Model{
        ...
        */
        public function count(){
            $c = M();
            $count = $c -> query("SELECT gattribute attr FROM food_goods {$this -> where()}");
        ...
        public function where(){
            ...
            if (isset($this->cid) && $this->cid) {
                ...
                $where[] = "gclassification IN (SELECT cid FROM `food_classify` WHERE `cpath` LIKE '{$curr['bpath']}%' OR cid = {$this->cid})";
            }
        ...
        private function defaultSQl(){
            return "SELECT gprice, gid,gclassification cid,gname,gpic,goldprice,gsellnums,gattribute attr FROM food_goods ";
        }
    }