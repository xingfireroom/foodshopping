...
class GoodsUpController extends Controller{
	...
	*/
	public function index(){
		...
		// 连贯操作查询数据
		$goodupdata = $goodsup->field('gprice,gid,gpic,gname,goldprice,goodnums,gsellnums,gissuetime')->where('issale=1')->order('gissuetime desc')->limit($limit)->select();
	...
	*/
	public function modGoods(){
		...
		$mod = M('goods');
		// 查询出要修改数据的字段
		$goodsdata = $mod->field('gprice,gid,gclassification,gname,gattribute,goldprice,goodnums,gpic,gintroduce,gspecifications,issale')->find($gid);
		// 拆分图片成数组
	...
	*/
	public function search(){
		...
		$limit = $pages->firstRow.','.$pages->listRows;
		// 连贯操作查询数据
		$goodupdata = $goodssearch->field('gprice,gid,gpic,gname,goldprice,goodnums,gsellnums,gissuetime')->where($map)->limit($limit)->select();
		/*遍历出来数据是二维数组，将遍历到的gpic多个地址，截取出来一个
		数据库中存放的gpic（$goodupdata['gpic']）字段是，多个图片地址用逗号链接的结果，这里用expode函数拆分字符串，取第一个展示到列表位置
		重新给$goodupdata['gpic']赋值为第一张图片的地址*/
		foreach ($goodupdata  as $key => $value) {
